...
function renderHeader() {
    ...
    return `
        <header class="sticky top-0 z-40 bg-white border-b border-sky-100 shadow-sm">
            ...
            ${step === 'quiz' ? '<div class="w-full bg-brand-surface border-t border-teal-100 px-4 py-2"><div class="mx-auto w-full max-w-4xl flex items-center justify-between gap-6"><span class="text-xs font-bold uppercase tracking-[0.18em] text-brand-mint">Tiến độ (Câu ' + (currentIndex + 1) + '/' + QUESTIONS.length + ')</span><div class="flex-1 max-w-sm h-3 overflow-hidden rounded-full bg-slate-100"><div class="h-full rounded-full bg-gradient-to-r from-brand-mint to-brand-blue transition-all duration-300" style="width: ' + (((currentIndex + 1) / QUESTIONS.length) * 100) + '%"></div></div></div></div>' : ''}
        </header>`;
}
